...
(return_statement
  ...
  local query = vim.treesitter.query.get("lua", "mq_plugin_is_disabled")
...
(return_statement
  ...
  local query = vim.treesitter.query.get("lua", "mq_plugin_has_enable_property")
...
local get_return_node = function(bufnr, tree)
  vim.treesitter.query.set(
    "lua",
    "mq_plugin_return_table_constructor",
    [[
(return_statement
  (expression_list
  ...
  local query = vim.treesitter.query.get("lua", "mq_plugin_return_table_constructor")
...
M.disable_plugin = function(prompt_bufnr)
  ...
  local uri = vim.uri_from_fname(entry.cwd .. entry[2])
  local bufnr = vim.uri_to_bufnr(uri)
  vim.fn.bufload(bufnr)

  local parsers = require "nvim-treesitter.parsers"
  local parser = parsers.get_parser(bufnr)
  local tree = parser:parse()[2]
  ...
  if enable_node == nil then
    ...
    start_line = return_node:start() + 3
    end_line = start_line
  else
  ...
  vim.notify("Plugin " .. entry[2] .. " Disabled", vim.log.levels.INFO, {})
  actions.close(prompt_bufnr)
...
M.enable_plugin = function(prompt_bufnr)
  ...
  vim.api.nvim_buf_set_lines(bufnr, enable_line, enable_line + 2, false, {})
  vim.notify("Plugin " .. entry[2] .. " Enabled", vim.log.levels.INFO, {})
  actions.close(prompt_bufnr)
  vim.api.nvim_buf_call(bufnr, function()
    vim.cmd "write"
  end)
